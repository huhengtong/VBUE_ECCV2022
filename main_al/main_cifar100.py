...
def main(context):
    ...
    dirpath = '/cache/5000initial_cifar/al_vib2_iter1'
...
#         model = resnet50(num_classes = 100)
    ...
    def compute_cLoss(data_idxs, dataset, model, ema_model):
        model.eval()
        ema_model.eval()
...
#         closs_list, idx_list = [], []
        for i, (input, (target, idx)) in enumerate(pred_loader):
            ...
            with torch.no_grad():
                ...
                output_ema,_,_ = ema_model(input)
...
#             print(torch.sum(c_loss, dim=1))
#             pdb.set_trace()
            closs_list[idx] = c_loss
        closs_list = closs_list[np.where(closs_list != 0)[0]]
        assert len(closs_list) == len(data_idxs)
        return closs_list
...
#         assert len(label_list) == len(data_idxs)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #    np.save('/cache/5000initial_cifar/al_vib2_iter1/label_epoch_iter1.npy', label_epoch)            
        #    np.save('/cache/5000initial_cifar/al_vib2_iter1/pseudo_list_iter1.npy', pseudo_list)
...
#     testdir = 'mini-imagenet/test'
    ...
    labeled_idxs_initial = read_dataset_initial(dataset, 50)
    labeled_idxs_al_iter1 = np.load('/cache/5000initial_cifar/al_vib2_iter1/idx_selected_iter1.npy')
    #labeled_idxs_al_iter2 = np.load('/cache/5000initial_cifar/al_vib2_iter2/idx_selected_iter2.npy')
    #labeled_idxs_al_iter3 = np.load('/cache/5000initial_cifar/al_vib1_iter3/idx_selected_iter3.npy')
    #labeled_idxs_al_iter4 = np.load('/cache/5000initial_cifar/al_vib1_iter4/idx_selected_iter4.npy')
