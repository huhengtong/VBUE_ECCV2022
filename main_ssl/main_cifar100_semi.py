...
def main(context):
    ...
    dirpath = '/dev/shm/10000initial_cifar100/iter1'
...
#         pred_list = torch.zeros(len(dataset.imgs))
        pseudo_list = torch.zeros(len(dataset.imgs), num_classes)
        label_list = torch.zeros(len(dataset.imgs)).long() +1000
...
#             return list_rank
        ...
        #vibration_ent = flip_vibration(entropy_list, -10, best_epoch-60, best_epoch)

#         ### compute the std
#         ent_rank = get_rankInfo(vibration_ent)
#         flip_rank = get_rankInfo(vibration_flip)
#         conf_rank = get_rankInfo(vibration_conf)
#         fused_feat = np.stack((ent_rank, flip_rank, conf_rank))
#         std_fused = np.std(fused_feat, axis=0)

        vibration_fused = 0.8*normlize(vibration_conf) + 0.2*normlize(vibration_flip) #+ 0.1*normlize(vibration_ent)
        return vibration_fused
    
    def run_training(dataset_eval, unlabeled_idxs, labels_training):        
        pseudo_list = torch.zeros((len(unlabeled_idxs), num_classes, args.epochs))
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # train for one epoch        
            train(train_loader, model, ema_model, optimizer, epoch, training_log, labels_training)
            
            if epoch >= 50:
                pseudo_list[:,:,epoch], label_epoch[:, epoch] = get_pc(unlabeled_idxs, dataset_eval, ema_model)   
            if epoch == args.epochs - 1:
                np.save('/dev/shm/10000initial_cifar100/iter1/label_epoch_iter.npy', label_epoch)         
                np.save('/dev/shm/10000initial_cifar100/iter1/pseudo_list_iter.npy', pseudo_list)
...
#         if iter == 0:
#             pseudo_labels_iter0 = np.load('/dev/shm/1000initial_cifar10/iter1/pseudo_list_iter0.npy')
#             labels_epoch_iter0 = np.load('/dev/shm/1000initial_cifar10/iter1/label_epoch_iter0.npy')
#         else:
        pseudo_labels_iter0 = np.load('/dev/shm/10000initial_cifar100/iter1/pseudo_list_iter.npy')
        labels_epoch_iter0 = np.load('/dev/shm/10000initial_cifar100/iter1/label_epoch_iter.npy')
        ...
        mean_vib = vibration_iter0.mean()
        print('iteration', iter, 'mean of vibration:', mean_vib)

        ### select the easy pseudo labels
        if iter >= 6:
            inds_easy = np.argsort(vibration_iter0)[0:21000]
        else:
            inds_easy = np.argsort(vibration_iter0)[0:15000+1000*iter]
        #inds_easy = np.argsort(vibration_iter0)[0:15000+1000*iter]
        ...
        targets_list = []
        for idx in range(len(dataset_eval.imgs)):
            path, (label_idx, idx) = dataset_eval.imgs[idx]
            #dataset.imgs[idx] = path, (label_idx, idx)
            targets_list.append(label_idx)
        targets_list = targets_list[unlabeled_idxs]
        acc_plabels = np.mean(np.asarray(targets_list)[inds_easy] == labels_epoch_iter0[inds_easy, best_epoch])
        
        pred_label_list = labels_epoch_iter0[:, best_epoch]
        acc_all = np.mean(np.asarray(targets_list) == pred_label_list)
        
        inds_conf = np.argsort(pseudo_labels_iter0[range(len(pred_label_list)), pred_label_list, best_epoch])[-len(inds_easy):]
        acc_conf = np.mean(np.asarray(targets_list)[inds_conf] == labels_epoch_iter0[inds_conf, best_epoch])
        print('iteration:', iter, 'accuracy of vibration:', acc_plabels, 'accuracy of all:', acc_all, 'accuracy of confidence:', acc_conf)
        ...
        return labels_training, mean_vib, acc_plabels, acc_all, acc_conf
        
    dataset_eval = data.read_cifar100()  
...
#     best_epoch = 126
    ...
    uncertainty_list, acc_vib_list, acc_all_list, acc_conf_list = [], [], [], []
    for iter in range(20):
        print('start training iter:', iter)
        #run_training(dataset_eval, unlabeled_idxs, weights_total, labels_training, weights_class)
        run_training(dataset_eval, unlabeled_idxs, labels_training)
        ckpt_path = '/dev/shm/10000initial_cifar100/iter1/best.ckpt'
        ...
        labels_training, mean_vib, acc_plabels, acc_all, acc_conf = get_weights(dataset_eval, labeled_idxs, unlabeled_idxs, best_epoch, iter)
        uncertainty_list.append(mean_vib)
        acc_vib_list.append(acc_plabels)
        acc_all_list.append(acc_all)
        acc_conf_list.append(acc_conf) 
    print('uncertainty list', uncertainty_list)
    print('acc of vib', acc_vib_list)
    print('acc of all', acc_all_list)
    print('acc of conf', acc_conf_list)
...
def train(train_loader, model, ema_model, optimizer, epoch, log, plabels_easy):
    global global_step
...
#         neg_labels = neg_labels_all[img_id]
        ...
        if len(inds) > 0:
            target[inds] = labels_easy[inds].long()         
...
if __name__ == '__main__':
    ...
    if not os.path.exists('/dev/shm/cifar100_train'):
